...
// includes, kernels
#include "vector_reduction_k_adv.cu"
...
// dimentis, excutes kernel function, and copy result of scan back
// to h_data.
...
float computeOnDevice(float* h_data, int num_elements)
{
  ...
  cudaMemcpy(d,h_data,size,cudaMemcpyHostToDevice);
  reduction<<<((num_elements/1024)+1),1024>>>(d,num_elements);
  float x;
  cudaMemcpy(h_data,d,size,cudaMemcpyDeviceToHost);
  cudaFree(d);
  return h_data[0];
